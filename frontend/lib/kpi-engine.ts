...
export async function updateKpis(): Promise<DashboardKpi> {
  ...
  const weightedDailyReturnPct = nav > 0 ? values.reduce((sum, asset) => sum + asset.change24hPct * (asset.value / nav), 0) : 0;
  ...
  const topAltcoin = values.filter((asset) => asset.bucket === 'MOONSHOT').sort((a, b) => b.value - a.value)[0];
